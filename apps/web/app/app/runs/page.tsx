    ...
    <div style={S.page}>
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"32px 24px" }}>
        ...
        <div style={{ ...S.card, overflow:"hidden" }}>
          <div style={{ display:"grid", gridTemplateColumns:"24px 1fr 120px 80px 80px 70px 80px 44px", borderBottom:"1px solid #1E3A5F" }}>
          ...
          {filtered.map((run, i) => {
            ...
            return (
              <div key={run.id} style={{ display:"grid", gridTemplateColumns:"24px 1fr 120px 80px 80px 70px 80px 44px", borderBottom:i<filtered.length-1?"1px solid #1E3A5F":"none", alignItems:"center", transition:"background .15s" }}
                ...
                {/* Task — clickable */}
                <Link href={`/app/runs/${run.id}`} style={{ padding:"12px 12px", minWidth:0, textDecoration:"none" }}>
                ...
                <div style={{ padding:"8px", display:"flex", justifyContent:"center" }}>
                  <button onClick={e => handleDelete(run.id, e)} disabled={deleting === run.id}
                    style={{ fontSize:11, color:"#FF4D6A", background:"transparent", border:"none", cursor:"pointer", opacity: deleting===run.id ? 0.4 : 0.6, padding:"4px 6px", borderRadius:4, transition:"opacity .15s" }}
                    title="Delete run"
                    onMouseEnter={e=>(e.currentTarget.style.opacity="1")}
                    onMouseLeave={e=>(e.currentTarget.style.opacity=deleting===run.id?"0.4":"0.6")}>
                    ✗
                  </button>
                </div>
              </div>
